...
A cifra de Vigenère foi implementada de raiz, suportando tanto a geração automática de uma \textit{Tabula Recta} padrão ($26 \times 26$) como o carregamento de tabelas personalizadas. O algoritmo utiliza o método auxiliar \texttt{\_extend\_key} para ajustar o comprimento da chave à mensagem.

\subsubsection{Cifragem e Decifragem}
...
\begin{enumerate}
    \item \textbf{Cifragem ($O(1)$):} O algoritmo calcula os índices da linha (chave) e coluna (texto) e acede diretamente à matriz.
    \item \textbf{Decifragem ($O(N)$):} O algoritmo identifica a linha através da chave, mas necessita de percorrer essa linha (pesquisa linear) para encontrar a posição do caráter cifrado e deduzir a coluna original.
\end{enumerate}
...
# Cifragem: Calculo direto das coordenadas
row = ord(key[i]) - ord('A')
col = ord(char) - ord('A')
...
# Decifragem: Identifica a linha e procura o carater
row = ord(key[i]) - ord('A')

for col in range(26):
    # Percorre a linha ate encontrar o carater cifrado
...
\begin{lstlisting}[caption={Regras de transformação Playfair (Cifragem)}, label={lst:playfair_logic}]
# Determina as coordenadas do par na matriz
